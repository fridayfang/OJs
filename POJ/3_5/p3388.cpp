...
void tarjan(int u,int fa){//无向连通图割点
    ...
    for(int t=head[u];t!=-1;t=es[t].nxt){
        ...
        if(low[v]>=dfn[u]&&fa!=u){cut[u]=1;}////u->v,v能到达的地方在u的下方
        }
    ...
    //db(u),db(fa),db(child);
    if(fa==u&&child>=2){cut[u]=1;}//对于根节点，只需树枝边>=2,是搜索树的树枝边
}
*/
void tarjan(int u,int fa){
    ...
    for(int t=head[u];t!=-1;t=es[t].nxt){
        int v=es[t].v;
        if(v!=fa&&!dfn[v]){
            child++;tarjan(v,u),low[u]=min(low[u],low[v]);
        }
        else if(v!=fa&&dfn[v]){low[u]=min(low[u],dfn[v]);}
        if(v!=fa&&low[v]>=dfn[u]){cut[u]=1;}
    }
